...
class Session(object):
    ...
    def create_permission_url(self, scope, redirect_uri, state=None):
        ...
        return "http://%s/admin/oauth/authorize?%s" % (self.url, urllib.parse.urlencode(query_params))
    ...
    def request_token(self, params):
        ...
        url = "http://%s/admin/oauth/access_token?" % self.url
    ...
    def __prepare_url(cls, url):
        if not url or (url.strip() == ""):
            return None
        url = re.sub("^http?://", "", url)
        shop = urllib.parse.urlparse("http://" + url).hostname
        if shop is None:
            return None
        idx = shop.find(".")
        if idx != -1:
