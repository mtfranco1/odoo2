...
class SaleOrder(models.Model):
    ...
    def _create_invoices(self, grouped=False, final=False, date=None):
        ...
        for order in self:
            if order.shopify_instance_id:
                res += super(SaleOrder,order.with_context(round=False))._create_invoices(grouped=grouped,
                                                                                         final=final, date=date)
            else :
                res += super(SaleOrder,order)._create_invoices(grouped=grouped,final=final, date=date)
        return res
