...
class SaleOrder(models.Model):
    ...
    def create_shopify_refund(self, refunds_data, total_refund):
        ...
        if self.amount_total == total_refund:
            ...
            move_reversal.new_move_ids.message_post(_(body="Credit note generated by Webhook as Order refunded in "
                                                           "Shopify."))
            return [True]
        return [3]
    ...
    def fulfilled_picking_for_shopify(self, pickings):
        ...
        for picking in pickings.filtered(lambda x: x.state not in ['cancel', 'done']):
            ...
            if picking.state == "done":
                picking.message_post(_(body="Picking is done by Webhook as Order is fulfilled in Shopify."))
